...
- `local_bridge_server.py`
  ...
  - 支持 `GET /open-folder`，可从插件直接打开导出目录。
...
- `browser-extension/`
  ...
  - 支持楼层范围输入与“打开输出目录”按钮。
  - 支持检测本地桥是否在线；安装 Windows 协议后，可从插件内自动拉起本地桥。
...
4. 如有需要，可在插件中填写“楼层范围”，只导出部分楼层。
5. 输出落地到 `cases/<topic_id>/`。
6. 导出成功后可直接点击插件里的“打开输出目录”。
7. 插件弹窗会保存最近导出记录，可手动清空。
...
注册后，插件在检测到本地桥未启动时，会尝试通过 `linuxdo-archive://start` 自动拉起本地桥。

如需卸载该协议：
```powershell
cd challenge-05-linuxdo-archive
powershell -NoProfile -ExecutionPolicy Bypass -File .\tools\windows\unregister-protocol.ps1
```
